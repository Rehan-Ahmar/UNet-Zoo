...
class BraTSDatasetUnet(Dataset):
    ...
    def __init__(self, dataset_folder, train=True, keywords=["P1", "1", "flair"], im_size=[128, 128], transform=None):
        ...
        masked_folder = folder + 'masked/'
        
        for file in os.listdir(images_folder):
            if file.endswith(".png") or file.endswith(".jpg"):
                masked_img_path = os.path.join(masked_folder, file)
                
                if not os.path.isfile(masked_img_path):
                    continue
                ...
                self.__mask.append(masked_img_path)
    ...
    def __getitem__(self, index):
        ...
        img = getImg(self.__im[index]).convert('L')
...
class TestDataset(Dataset):
    ...
    def __init__(self, dataset_folder, im_size=[128, 128], transform=None):
